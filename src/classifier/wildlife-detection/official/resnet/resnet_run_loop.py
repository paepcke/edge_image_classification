...
def resnet_model_fn(features, labels, mode, model_class,
  ...
  accuracy_top_5 = tf.metrics.mean(tf.nn.in_top_k(predictions=logits,
                                                  ...
                                                  k=5,
                                                  name='top_5_op'))
  mean_class_accuracy = tf.metrics.mean_per_class_accuracy(labels, predictions['classes'], num_classes=3) #TODO REMOVE HARDCODING
  #class0_acc = per_class_accuracy[0][0], per_class_accuracy[1][0]
 # class1_acc = per_class_accuracy[0][1], per_class_accuracy[1][1]
  #class2_acc = per_class_accuracy[0][2], per_class_accuracy[1][2]
  cond = tf.equal(labels, tf.constant(0))
  indices = tf.where(cond)
  class0_acc = tf.metrics.accuracy(tf.gather_nd(labels, indices), tf.gather_nd(predictions['classes'], indices))
  
  cond = tf.equal(labels, tf.constant(1))
  indices = tf.where(cond)
  class1_acc = tf.metrics.accuracy(tf.gather_nd(labels, indices), tf.gather_nd(predictions['classes'], indices))
  
  cond = tf.equal(labels, tf.constant(2))
  indices = tf.where(cond)
  class2_acc = tf.metrics.accuracy(tf.gather_nd(labels, indices), tf.gather_nd(predictions['classes'], indices))


  metrics = {'accuracy': accuracy,
             'accuracy_top_5': accuracy_top_5,
             'coyote_acc': class0_acc,
             'human_acc' : class1_acc,
             'lion_acc' : class2_acc,
             'mean_class_accuracy': mean_class_accuracy}
  # Create a tensor named train_accuracy for logging purposes
  tf.identity(accuracy[1], name='train_accuracy')
  tf.identity(accuracy_top_5[1], name='train_accuracy_top_5')
  tf.identity(class0_acc[1], name='coyote_train_accuracy')
  tf.identity(class1_acc[1], name='human_train_accuracy')
  tf.identity(class2_acc[1], name='lion_train_accuracy')
  tf.summary.scalar('train_accuracy', accuracy[1])
  tf.summary.scalar('train_accuracy_top_5', accuracy_top_5[1])
  tf.summary.scalar('coyote_train_accuracy', class0_acc[1])
  tf.summary.scalar('human_train_accuracy', class1_acc[1])
  tf.summary.scalar('lion_train_accuracy', class2_acc[1])
  ...
  if flags_obj.export_dir is not None:
    ...
    if flags_obj.image_bytes_as_serving_input:
      ...
      input_receiver_fn = export.build_tensor_serving_input_receiver_fn(
          shape, batch_size=1, dtype=export_dtype)
